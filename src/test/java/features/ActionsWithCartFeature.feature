Feature: Action with cart
  ...
  Scenario Outline: Check add product to cart
    When User inputs '<SEARCH_WORD>' into search field with action functionality
    ...
    Examples:
      ...
      | xiaomi      | XIAOMI MiJia Automatic Foam Soap MJXSJ01XW/MJXSJ03XW | 1              |

  Scenario Outline: Check remove product from cart
    When User inputs '<SEARCH_WORD>' to search field and press enter
    When User adds into the cart product with '<PRODUCT_TITLE>' title
    ...
    Examples:
      ...
      | xiaomi      | XIAOMI MiJia Automatic Foam Soap MJXSJ01XW/MJXSJ03XW | Корзина пуста |
  ...
  Scenario Outline: Check subtotal price of elements in cart
    ...
    Examples:
      ...
      | xiaomi      | XIAOMI MiJia Automatic Foam Soap MJXSJ01XW/MJXSJ03XW | IP-камера Xiaomi YI 1080p Home White YYS.2016 | IP-камера Xiaomi YI Dome X 360° 1080P White YYS.3017 |